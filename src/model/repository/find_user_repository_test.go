...
func TestUserRepository_FindUserByEmail(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		mock.ExpectQuery(regexp.QuoteMeta("SELECT email,password,name,age,id FROM users WHERE email = ? AND password = ?")).
			WithArgs("test@test.com","test").
			WillReturnRows(sqlmock.NewRows([]string{"email", "password", "name", "age", "id"}).
			AddRow("test@test.com", "test", "test", uint8(90), 1))

		repo := NewUserRepository(db)
		userDomain, err := repo.FindUserByEmailAndPassword("test@test.com","test")
